...
static void MX_TIM10_Init(void) {
    ...
    if (HAL_TIM_IC_ConfigChannel(&htim10, &sConfigIC, TIM_CHANNEL_1) !=
        HAL_OK) {
        Error_Handler();
    }
...
static void MX_GPIO_Init(void) {
    ...
    HAL_GPIO_WritePin(GPIOG,
                      DIRA_ROUE_GAUCHE_Pin | DIRB_ROUE_GAUCHE_Pin |
                          TRIG_GAUCHE_AR_Pin | USB_PowerSwitchOn_Pin |
                          TRIG_GAUCHE_AV_Pin,
                      GPIO_PIN_RESET);

    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOB,
                      DIRA_ROUE_DROITE_Pin | DIRB_ROUE_DROITE_Pin |
                          DIRA_LEVAGE_Pin | DIRB_LEVAGE_Pin,
                      GPIO_PIN_RESET);

    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOC, PWM_RETOURNE_4_Pin | PWM_ECARTEMENT_Pin,
                      GPIO_PIN_RESET);

    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(TRIG_DROITE_AV_GPIO_Port, TRIG_DROITE_AV_Pin,
                      GPIO_PIN_RESET);
    ...
    /*Configure GPIO pins : DIRA_ROUE_GAUCHE_Pin DIRB_ROUE_GAUCHE_Pin
       TRIG_GAUCHE_AR_Pin USB_PowerSwitchOn_Pin TRIG_GAUCHE_AV_Pin */
    ...
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(TRIG_DROITE_AV_GPIO_Port, &GPIO_InitStruct);
